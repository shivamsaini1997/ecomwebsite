...
import { OrderData } from '../Supplier/Pages/OrderData';
// import OrderData from 'supplier/OrderData'
...
export default function AdvancedFilterDemo() {
    ...
    useEffect(() => {
        OrderData.getCustomersMedium().then((data) => {
    ...
    const renderHeader = () => {
        return (
            ...
            <Button type="button" icon="pi pi-filter-slash" className='Approved btn-data' label="Paid (86)" />
            <Button type="button" icon="pi pi-filter-slash"className='Declined btn-data' label="Cancelled (9)" />

            <Button type="button" icon="pi pi-filter-slash" className='Approved btn-data' label="Delivered (87)" />
            <Button type="button" icon="pi pi-filter-slash"className='dis btn-data' label="Dispatched (27)" />
    ...
    return (
        <div className="card card-table">
            <DataTable value={customers} paginator showGridlines rows={10} loading={loading} dataKey="id"  className='mb-5'
                    filters={filters} globalFilterFields={['S.no','country.name', 'representative', 'balance', 'Created Date', 'Quantity', 'Availability','Action']} header={header}
                    emptyMessage="No customers found.">
                <Column field="name" header="S.no" filterPlaceholder="Search by name" style={{ minWidth: '3rem' }} />

                <Column header="Order ID" filterField="country.name" style={{ minWidth: '6rem' }} body={countryBodyTemplate}
                filterPlaceholder="Search by country" filterClear={filterClearTemplate} 
                    ...
                    <Column header="Billing Name" filterField="representative" showFilterMatchModes={false} filterMenuStyle={{ width: '14rem' }} style={{ minWidth: '12rem' }}
                    body={representativeBodyTemplate}  filterElement={representativeFilterTemplate} />


                <Column header="Date" filterField="date" dataType="date" style={{ minWidth: '8rem' }} body={dateBodyTemplate}   filterElement={dateFilterTemplate} />
                <Column header="Totle" filterField="balance" dataType="numeric" style={{ minWidth: '6rem' }} body={balanceBodyTemplate}  filterElement={balanceFilterTemplate} />


                <Column field="Quantity" header="Payment Status" filterMenuStyle={{ width: '8rem' }} style={{ minWidth: '8rem' }} filterElement={QuantityFilterTemplate} />

                <Column field="Paymentmethod" header="Payment method" filterMenuStyle={{ width: '8rem' }} style={{ minWidth: '8rem' }} filterElement={QuantityFilterTemplate} />
